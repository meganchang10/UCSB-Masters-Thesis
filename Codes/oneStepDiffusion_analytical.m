function [u_n] = oneStepDiffusion_analytical(u_n,phi,m,n,D,S,BC_interface,BC_domain,x,y,dx,dy,dt,t)
...
for j = 2:n-1
    for i = 2:m-1
        if phi(i,j) == 0
            u_n(i,j) = BC_interface(x(i),y(j),t);
        elseif phi(i,j) < 0
            %LEFT
        ...
        if phi(i,j) < 0
            ...
            if i > 2
                ...
                elseif phi(i,j)*phi(i-1,j) < 0
                    theta = abs(phi(i,j))/dx;
                    ghost_n   = ghost_n   + (1/theta)*(D*dt/dx^2)*BC_interface(x(i)-theta*dx,y(j),t);
                    ghost_nm1 = ghost_nm1 + (1/theta)*(D*dt/dx^2)*BC_interface(x(i)-theta*dx,y(j),t-dt);
                    Center = Center + ((1-theta)/theta)*(D*dt/dx^2);
                elseif phi(i,j)*phi(i-1,j) == 0
                    ghost_n   = ghost_n   + D*dt/dx^2*BC_interface(x(i-1),y(j),t);
                    ghost_nm1 = ghost_nm1 + D*dt/dx^2*BC_interface(x(i-1),y(j),t-dt);
                end
            else
                ghost_n   = ghost_n   + D*dt/dx^2*BC_domain(x(i),y(j),t);
                ghost_nm1 = ghost_nm1 + D*dt/dx^2*BC_domain(x(i),y(j),t-dt);
            end
            ...
            if j > 2
                ...
                elseif phi(i,j)*phi(i,j-1) < 0
                    theta = abs(phi(i,j))/dy;
                    ghost_n   = ghost_n   + (1/theta)*(D*dt/dy^2)*BC_interface(x(i),y(j)-theta*dy,t);
                    ghost_nm1 = ghost_nm1 + (1/theta)*(D*dt/dy^2)*BC_interface(x(i),y(j)-theta*dy,t-dt);
                    Center = Center + ((1-theta)/theta)*(D*dt/dy^2);
                elseif phi(i,j)*phi(i,j-1) == 0
                    ghost_n   = ghost_n   + D*dt/dy^2*BC_interface(x(i),y(j-1),t);
                    ghost_nm1 = ghost_nm1 + D*dt/dy^2*BC_interface(x(i),y(j-1),t-dt);
                end
            else
                ghost_n   = ghost_n   + D*dt/dy^2*BC_domain(x(i),y(j),t);
                ghost_nm1 = ghost_nm1 + D*dt/dy^2*BC_domain(x(i),y(j),t-dt);
            end
