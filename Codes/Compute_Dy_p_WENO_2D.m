function [Dy_p] = Compute_Dy_p_WENO_2D(u,dy,m,n,BCy)
...
% Dy_p, this is the side that would include j and j+1. WENO schemes choose
% between three possible stencils that can range from point j-2 to j+3 for
% Dy_p, selecting the stencil that will create the smoothest solution and
...
% each stencil u_y1, u_y2, and u_y3 will contribute to the final Dy_p
% solution.

Dy_p = zeros(m,n);
% Pad u with valid values that reflect the boundary conditions at the walls
% in the y-direction to account for each stencil size which extends from
% u(i,j-2) to u(i,j+3) in the Dy_p upwind direction
...
for i = 1:m
    ...
    if (BCy==0)          % Padd u with Linear Extrapolation BC
        ...
        u_padd(i,n+3) = 2*u_padd(i,n+2) - u_padd(i,n+1);
...
for i = 1:m
    for j = 3:n+2
        ...
        d1 = (u_padd(i,j+3) - u_padd(i,j+2))/dy;
        d2 = (u_padd(i,j+2) - u_padd(i,j+1))/dy;
        d3 = (u_padd(i,j+1) - u_padd(i  ,j))/dy;
        d4 = (u_padd(i  ,j) - u_padd(i,j-1))/dy;
        d5 = (u_padd(i,j-1) - u_padd(i,j-2))/dy;
        ...
        % Dy_p construction
        Dy_p(i,j-2) = omega1*u_y1 + omega2*u_y2 + omega3*u_y3;
    end
end
end