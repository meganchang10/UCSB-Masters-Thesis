function [Dx_p] = Compute_Dx_p_WENO_2D(u,dx,m,n,BCx)
...
% Dx_p, this is the side that would include i and i+1. WENO schemes choose
% between three possible stencils that can range from point i-2 to i+3 for
% Dx_p, selecting the stencil that will create the smoothest solution and
...
for j = 1:n
    ...
    if (BCx==0)          % Padd u with Linear Extrapolation BC
        ...
        u_padd(m+3,j) = 2*u_padd(m+2,j) - u_padd(m+1,j);
...
for j = 1:n
    for i = 3:m+2
        ...
        d1 = (u_padd(i+3,j) - u_padd(i+2,j))/dx;
        d2 = (u_padd(i+2,j) - u_padd(i+1,j))/dx;
        d3 = (u_padd(i+1,j) - u_padd(i  ,j))/dx;
        d4 = (u_padd(i  ,j) - u_padd(i-1,j))/dx;
        d5 = (u_padd(i-1,j) - u_padd(i-2,j))/dx;
        ...
        % Dx_p construction
        Dx_p(i-2,j) = omega1*u_x1 + omega2*u_x2 + omega3*u_x3;
    end
end
end